...
POSTS_DIR = "_posts"
MIN_PRODUCTS = 2  # Seuil minimal de produits pour créer un article sur un artiste
...
def parse_item(content):
    """ Tente d'extraire des données JSON ou Frontmatter YAML """
    try:
...
def load_products_from_repo():
    print(f"1. Analyse du dépôt https://github.com/{REPO_OWNER}/{REPO_NAME}...")
    
    tree_data = None
    branch_used = "main"
    for branch in ["main", "master"]:
        url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/git/trees/{branch}?recursive=1"
        tree_data = fetch_json(url, headers={"User-Agent": "Python-Script"})
        if tree_data and "tree" in tree_data:
            branch_used = branch
            break

    if not tree_data or "tree" not in tree_data:
        print("Erreur : Impossible de lire l'arborescence du dépôt.")
        return []
    ...
    for f in files:
        path = f["path"]
        if path.endswith((".json", ".md", ".html")) and not path.startswith("."):
            raw_url = f"https://raw.githubusercontent.com/{REPO_OWNER}/{REPO_NAME}/{branch_used}/{path}"
            content = fetch_text(raw_url)
            if content:
                items = parse_item(content)
                for item in items:
                    artist = get_field(item, ["artist", "artiste", "band", "author", "groupe"])
                    ...
                    if artist and title:
                        products.append({
                            "artist": artist,
                            "title": title,
                            "format": get_field(item, ["format", "media", "support", "type"], "Support d'occasion"),
                            "price": get_field(item, ["price", "prix"], ""),
                            "url": get_field(item, ["url", "link", "lien", "buy_url"], f"https://{REPO_OWNER.lower()}.github.io/{REPO_NAME}/"),
                            "description": get_field(item, ["description", "body", "summary"], "")[:200]
                        })

    print(f"-> {len(products)} fiches produits actives extraites.")
    return products

def clean_dead_links(valid_products):
    """ Nettoie les liens morts dans les articles déjà publiés """
    if not Path(POSTS_DIR).exists():
        return

    print("2. Vérification et nettoyage des liens morts dans les anciens articles...")
    ...
    for post_file in Path(POSTS_DIR).glob("*.md"):
        ...
        def link_replacer(match):
            ...
            # Si le lien pointe vers le site 1 mais n'est plus dans la liste active
            if REPO_NAME in url and url not in valid_urls and url != fallback_store_url:
                print(f"   [Lien mort corrigé dans {post_file.name}] : {url}")
                return f"[{text} (Épuisé - Voir le catalogue)]({fallback_store_url})"
            return match.group(0)

        new_content = re.sub(r'\[([^\]]+)\]\((https?://[^\)]+)\)', link_replacer, content)
...
"""
    ...
    payload = json.dumps({
        "messages": [
            {"role": "system", "content": "Tu es un spécialiste de la musique d'occasion et de la rédaction SEO."},
...
def main():
    ...
    # Nettoyage automatique des liens sur les articles déjà publiés
    ...
    print(f"3. Génération de l'article pour {target_artist} ({len(target_products)} référence(s))...")
